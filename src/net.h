...
CNode* FindNode(const CService& ip, bool verified = false);
CNode* ConnectNode(CAddress addrConnect, const char *strDest = NULL, bool verified = false);
void MapPort();
...
public:
    ...
    CNode(SOCKET hSocketIn, CAddress addrIn, std::string addrNameIn = "", bool fInboundIn=false, bool fTorIn=false) : vSend(SER_NETWORK, MIN_PROTO_VERSION), vRecv(SER_NETWORK, MIN_PROTO_VERSION)
    {
        nServices = 0;
        hSocket = hSocketIn;
        nLastSend = 0;
        ...
        nMessageStart = -1;
        addr = addrIn;
        addrName = addrNameIn == "" ? addr.ToStringIP() : addrNameIn;
        nVersion = 0;
        strSubVer = "";
        fOneShot = false;
