...
void SendCoinsByDelegateDialog::on_sendButton_clicked()
{
    ...
    if (!model->getOptionsModel() || !model->getOptionsModel()->getCoinControlFeatures())
        sendstatus = model->sendCoinsByDelegate(recipients);
    else
        sendstatus = model->sendCoinsByDelegate(recipients, CoinControlDialog::coinControl);

    switch(sendstatus.status)
    {
    case WalletModel::InvalidAddress:
        QMessageBox::warning(this, tr("Send Coins By Delegate"),
    ...
    case WalletModel::TransactionCreationFailed:
        QMessageBox::warning(this, tr("Send Coins By Delegate"),
            ...
            QMessageBox::Ok, QMessageBox::Ok);
        break;
    case WalletModel::DelegateNotFound:
        QMessageBox::warning(this, tr("Send Coins By Delegate"),
            tr("No delegate found to process transaction"),
            QMessageBox::Ok, QMessageBox::Ok);
        break;
...
void SendCoinsByDelegateDialog::coinControlChangeEdited(const QString & text)
{
    if (model)
    {
        ...
        else if (!CBitcoinAddress(text.toStdString()).IsValid())
        {
            ...
            ui->labelCoinControlChangeLabel->setText(tr("WARNING: Invalid MillenniumCoin address"));
        }
        else
        {
