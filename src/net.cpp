...
CNode* FindNode(const CService& addr, bool verified) //TODO find out abt verified
{
        LOCK(cs_vNodes);
        BOOST_FOREACH(CNode* pnode, vNodes)
...
CNode* ConnectNode(CAddress addrConnect, const char *pszDest, bool verified)
{
    if (pszDest == NULL) {
        if (IsLocal(addrConnect))
            return NULL;

        // Look for an existing connection ***TODO verified??
        CNode* pnode = FindNode((CService)addrConnect, verified);
        if (pnode)
        {
            pnode->AddRef();
            return pnode;
    ...
    if (pszDest ? ConnectSocketByName(addrConnect, hSocket, pszDest, GetDefaultPort()) : ConnectSocket(addrConnect, hSocket))
    {
        ...
        // Add node
        CNode* pnode = new CNode(hSocket, addrConnect, pszDest ? pszDest : "", false, addrConnect.IsTor());
        pnode->AddRef();
...
void PushOffChain(
    ...
    CAddress destination(CService(parsed, GetDefaultPort()));
...
void  InitializeDelegateBind(
    ...
    CBitcoinAddress recovery_address;
