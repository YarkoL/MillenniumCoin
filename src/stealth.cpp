...
int StealthSecret(ec_secret& secret, ec_point& pubkey, const ec_point& pkSpend, ec_secret& sharedSOut, ec_point& pkOut)
{
    ...
    send:
        secret (c) = ephem_secret (e), pubkey = scan_pubkey (Q)

    receive:
        secret (c) = scan_secret (d), pubkey = ephem_pubkey (P)


    Q = public scan key (EC point, 33 bytes)  = dG
    d = private scan key (integer, 32 bytes)

    R = public spend key = fG
    f = private spend key
    ...
    Sender (has Q and R, not d or f):
        ...
        calculate public ephem key (embed as metadata)
        P = eG

        calculate stealth pubkey
        R' = R + cG
